...
const CreateTreatyForm = () => {
  ...
  const [addDeduction] = useMutation(ADD_DEDUCTION_TO_TREATY, {
    ...
    onCompleted({ createTreatyProgramAssociateDeductions }) {
      // refetch({ variables: { id: insurer_id } });
      _form.setValue(
        "treaty_associate_deductionstreaty_associate_deduction_id",
  ...
  useEffect(() => {
    if (limitLayers && limitLayers.length > 0) {
      const firstLayer = _.first(limitLayers);
      if (firstLayer.min_rate) {
        //calculate m and d premium
      }
    }
  }, [limitLayers]);

  // useEffect(() => {
  //   if (limitLayers && limitLayers.length > 1) {
  //     const layers = limitLayers.shift();
  //     layers.map((layer, index) => {
  //       const lastLayer = limitLayers[index];
  //       return {
  //         ...layer,
  //         deductible: parseFloat(lastLayer.deductible) + parseFloat(lastLayer.limit),
  //       };
  //     });
  //   }
  // }, [limitLayers]);
  ...
  return (
    <div className="page-content">
      <form
        className="ml-4 mr-4"
        onSubmit={_form.handleSubmit(onSubmitTreatyForm)}
      >
        ...
        <div className={styles.card_header}>
          ...
          <Alert variant="danger">
            ...
            <p>
              {" "}
              N.B. Deductions are set for a treaty period. Make sure the clock
              on your personal pc is up to date to allow the system to auto-fill
          ...
          <fieldset className="border p-2 mb-2">
            ...
            <table className="table">
              <tbody>
                <tr>
                  <td>Name</td>
                  <td>{insurerData?.insurer?.insurer_company_name}</td>
                </tr>
              </tbody>
            </table>
          </fieldset>
            ...
            <div className="col-md-6">
              <div className="form-group">
                ...
                {errors.withholding_tax && (
                  <p className="text-danger">
                    {errors.withholding_tax.message}
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="col-md-6">
              <div className="form-group">
                ...
                {errors.treaty_period_to && (
                  <p className="text-danger">
                    {errors.treaty_period_to.message}
                  </p>
                )}
              </div>
            </div>
