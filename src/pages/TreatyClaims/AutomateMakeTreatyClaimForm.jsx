...
const AutomateMakeTreatyClaimForm = ({ treaty, setShow }) => {
    ...
    const onUpload = values => {
        const custom_headers = JSON.stringify(state.map((el, key) => ({
        ...
        <form onSubmit={handleSubmit(onUpload)}>
            <div className={styles.card_header}>
                ...
                <div className="alert alert-info">
                    ...
                    <ul>
                        <li>Upload handles only <strong>Excel (.xls, .csv, .xsls)</strong> files</li>
                        <li>Provide Corresponding Names of expected Columns if you have different names on the file to be uploaded</li>
            ...
            <div onClick={() => setUseCUstomeHeaders(prev => !prev)} className="row mx-1 mb-2">
            ...
            <div className="row mt-3">
                <div className="col-md-6">
                    <Input ref={register({ required: false })} label="Header start at" name="start_at" type="number" placeholder="headers start at" />
                </div>
                <div className="col-md-6">
                    <Select ref={register({ required: 'Required' })} options={layers} label="Layer Number" name="layer_number" />
                    <ErrorMessage errors={errors} name="layer_number" />
                    <Input type="hidden" name="treaty_id" value={treaty?.treaty_id} ref={register({ required: true })} />
                </div>
            </div>
            ...
            <div className="row mt-2">
                <div className="col-md-12">
                    <Input label="Select File" ref={register({ required: 'Required' })} name="file" type="file" accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel" />
