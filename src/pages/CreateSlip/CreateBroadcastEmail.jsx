...
function CreateBroadcastEmail({ offer_id, toggle, closed, noOfReinsurers = 0, noOfAssociates = 0 }) {
    ...
    const [sendmail, { loading: mailSending }] = useMutation(SEND_OFFER_AS_BROADCAST, { refetchQueries: [{ query: SINGLE_OFFER, variables: { offer_id } }] });
    ...
    const handleSubmitSendMail = ({ subject, copied_emails }) => {
        ...
        const data = {
            ...
            subject,
            copied_emails: copied_emails.length ? [...copiedMails.map(e => e.label)] : [],
            attachment: [...files]
        };
        setData(data);
        swal({
    ...
    useEffect(() => {
        if (responseData) {
            ...
            const list = responseData.associates.map(associates => ({
                ...
                status: <span className="badge badge-success" style={{ letterSpacing: 3 }}>Sent</span>
            }))
    ...
    return (
        <>
            <div className={styles.card_header}>
                <h2 className={styles.card_title}>Send Offer</h2>
                <Alert variant="danger">
                    <strong>Send offer to {noOfReinsurers} reinsurers with {noOfAssociates} Associates</strong>
            ...
            <Modal centered size="lg" show={showModal} onHide={() => setShowModal(!showModal)}>
                <Modal.Header>Record found</Modal.Header>
                <Modal.Body>
                    <Alert variant="danger">
                        <p>This offer has been sent to {receivedReps.length} associates along with the following attachments.</p>
                        <p><strong>Will you like to include them in this email ?</strong></p>
                    </Alert>
                    <h6>Mail received by</h6>
                    <Datatable entries={2} columns={responseFound} data={receivedReps} />
                    {responseData && responseData.file_listing.length ? <fieldset className="border p-1">
                        <legend className="w-auto" style={{ fontSize: 14, fontWeight: "bold" }}>Attachments</legend>
                        <div style={{ height: 150, overflow: "scroll" }}>
                            <ul>
                                {responseData && responseData.file_listing.map((file, key) => <li key={key}>
                        ...
                        <div className="form-check">
                            <input type="checkbox" className="form-check-input" id="exampleCheck1" onChange={e => setInclude_attachment(e.target.checked)} />
                            <label className="form-check-label" htmlFor="exampleCheck1">Include these attachment(s) to email</label>
                        </div>
                    </fieldset> : null}
                </Modal.Body>
                <Modal.Footer>
