            ...
            <div className="row">
                <table className="table table-borderless">
                    <tbody>
                        ...
                        <tr>
                            <td>Created On</td>
                            <td><strong>{new Date(data.system_notification.created_at).toDateString()}</strong></td>
                        </tr>
                    </tbody>
                </table>
            </div>

            {business && business?.business_details?.length && <div className="row">
                <div className="col-md-12">
                    <p>Business Details</p>
                </div>
                {JSON.parse(business?.business_details)?.map((detail, key) => <Detail key={key} detail={detail} />)}
            </div>}
        </div>
    )
}
