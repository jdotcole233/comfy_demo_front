...
const Converter = ({ available_cuurencies, formData, onGenerateFinished }) => {
    ...
    useEffect(() => {
        if (available_cuurencies) {
            setConvertFrom(prev => [...available_cuurencies.map(el => ({
                currency: el,
                rate: el === currency ? 1 : ""
            }))])
        }
    ...
    return (
        <div>
            {available_cuurencies.length ? <button onClick={() => setShowModla(c => !c)} type="button" className="font-size-16 btn btn-sm waves-effect waves-light w-md btn-warning">
            ...
            <Modal size="lg" show={showModla} onHide={() => setShowModla(c => !c)}>
                ...
                <Modal.Body>
                    <div className="row alert alert-danger">
                        All currency values in the brokerage statement will be converted based on the specified rate(s).
                    </div>
                    <form onSubmit={handleSubmit} className="row">
                        ...
                        {currency && convert_from.map((cf, k) => (
                            <div className="row col-md-12 mb-1" key={k}>
                                <div className="col-md-5">
                                    <label htmlFor="from">{cf.currency}</label>
                                    <input type="text" value={1} className="form-control" placeholder={cf} readOnly />
                                </div>
                                ...
                                <div className="col-md-6">
                                    ...
                                    <input type="number" name={k} value={currency === cf.currency ? 1 : cf.rate} onChange={handleChange} className="form-control" placeholder={currency ? currency : "?"} required />
                                </div>
                            </div>
                        ))}
                        <div className="col-md-12 d-flex justify-content-end py-3">
                            <button disabled={!currency} className="btn btn-sm w-md waves-effect waves-light btn-warning">
                                {loading && <i class="bx bx-hourglass bx-spin mr-2"></i>}
                                Convert
                                </button>
                        </div>
                    </form>
