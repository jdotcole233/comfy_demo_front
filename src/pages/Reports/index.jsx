...
const parseReportDate = (from, to) => {
  ...
  const _to = moment(to);
  const parsedDate = `${_from.format("DD/MM/YYYY")} - ${_to.format(
    "DD/MM/YYYY"
  )}`;
  return parsedDate;
};
...
function Reports() {
  ...
  const { data: classOfbusiness, loading: classOfBusinessLoading } = useQuery(
    FETCH_CLASS_OF_BUSINESS
  );
  ...
  const printTable = () => {
    ...
    doc.text(
      `Brokerage Statement as at ${new Date().toLocaleDateString()}`,
      5,
      10
    );
    doc.autoTable({
  ...
  useEffect(() => {
    if (classOfbusiness) {
      const _classOfbusiness = classOfbusiness.classOfBusinesses.map((el) => ({
        label: el.business_name,
        value: el,
      }));
      setClassOfBusinessData([
        { label: "All", value: { class_of_business_id: " " } },
        ..._classOfbusiness,
      ]);
    }
  }, [classOfbusiness]);
  ...
  useEffect(() => {
    if (reinsurers) {
      const _reinsurers = reinsurers.reinsurers.map((el) => ({
        label: el.re_company_name,
        value: el,
      }));
      setReinsurersData([
        { label: "All", value: { re_abbrv: "All", reinsurer_id: " " } },
        ..._reinsurers,
      ]);
    }
  }, [reinsurers]);
  ...
  useEffect(() => {
    if (insurers) {
      const _insurers = insurers.insurers.map((el) => ({
        label: el.insurer_company_name,
        value: el,
      }));
      setInsurersData([
        { label: "All", value: { insurer_abbrv: "All", insurer_id: " " } },
        ..._insurers,
      ]);
    }
  }, [insurers]);
  ...
  useEffect(() => {
    if (filters.length) {
      setContainsClosed(
        filters.some((el) => el.label === "CLOSED" || el.label === "ALL")
      );
    } else {
  ...
  const handleChooseFilter = (value) => {
    if (value && value.some((el) => el.label === "ALL")) {
      ...
      setValue(
        "offer_status",
        value.filter((val) => val.label === "ALL")
      );
      return;
    }
    console.log(value);
  ...
  const handleChooseBusiness = (value) => {
    if (value && value.some((el) => el.label === "All")) {
      setClassOfBusiness(value.filter((val) => val.label === "All"));
      setValue(
        "business_id",
        value.filter((val) => val.label === "All")
      );
        ...
        <div className="card">
          <div className="card-body">
            <div className="col-md-12">
              <div className="row">
                <h6 className="text-muted font-weight-medium">
                  Top five businesses
                </h6>
              </div>
              <Chart
                ...
                options={
                  !pieChartLabels
                    ? options
                    : { labels: pieChartLabels, pieChartSeries }
                }
                series={pieChartLabels ? pieChartSeries : options.series}
              />
            </div>
          </div>
        </div>
          ...
          <div className="card-body">
            <div className="row d-flex justify-content-between px-3">
              <p className="text-muted font-weight-medium">
                Brokerage Statement/Reporting{" "}
              </p>
              <Converter
                available_cuurencies={returnedCurrencies}
                formData={formData}
                onGenerateFinished={handleSuccessFulGeneration}
              />
            </div>
            <form onSubmit={handleSubmit(generateReport)} className="row">
              ...
              <div className="col-md-6">
                <div className="form-group">
                  ...
                  <Selector
                    ...
                    onChange={handleChoosePeriodStatus}
                    isMulti
                  ...
                  <input
                    ...
                    className="form-control"
                  />
                  {errors.period_status && (
                    <p className="text-danger">
                      {errors.period_status.message}
                    </p>
                  )}
                </div>
              </div>
              ...
              {containsClosed && (
                <div className="col-md-12">
                  <div className="form-group">
                    ...
                    <Selector
                      value={paymentStatus}
                      onChange={handleChoosePaymentStatus}
                      isMulti
                      placeholder="Select filter(s)"
                    ...
                    <input
                      type="hidden"
                      ref={register({
                        required: containsClosed ? "Required" : false,
                      })}
                      name="payment_status"
                      value={paymentStatus}
                      id=""
              ...
              <div className="form-group col-md-12">
                {!loading ? (
                  <button
                    type="submit"
                    className="btn btn-block btn-sm btn-primary waves-effect waves-light"
                  >
                    Generate report
                  </button>
                ) : (
