  ...
  return (
    ...
    >
      <div
        ...
        ref={chatBoxRef}
        className="border-form p-2"
      >
        {comments?.map((comment, id) => (
          <Pill comment={comment} key={id} />
        ))}
        {userComments.map((comment, id) => (
          <NewMessagePill comment={comment} key={id} />
        ))}
      </div>
      <div className="">
...
const NewMessagePill = ({ comment }) => {
  ...
  // console.log(state)
  return (
    <div div className={`chat-pill`}>
      <div className="abbrv-row">
        <div alt="Avatar" className={`abbrv-right`}>
      ...
      <span className={`time-right`}>{moment().fromNow()}</span>
    </div>
  );
};
...
const Pill = ({ comment }) => {
  ...
  return (
    <div className={`chat-pill ${me ? "" : "darker"}`}>
      <div className="abbrv-row">
        <div alt="Avatar" className={`${me ? "abbrv-right" : "abbrv"}`}>
      ...
      <span className={`${me ? "time-right" : "time-left"}`}>
