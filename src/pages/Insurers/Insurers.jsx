...
function Insurers() {
  ...
  return (
    <div className="page-content">
      ...
      {loading && <Loader />}
      {insurersInPages.length ? (
        <div className="row m-2">
          {insurersInPages[activePage].map((el, id) => (
            <Insurer
              key={id}
              openManagerDrawer={handleShowAddManger}
              data={el}
            />
          ))}
        </div>
      ...
      >
        <AddInsurer
          toggle={() => setshowAddInsurer(!!0)}
          closed={!showAddInsurer}
        />
      </Drawer>
      <Drawer
        width="40%"
        isvisible={showAddManager}
        toggle={() => setshowAddManager(!!0)}
      >
        <AddManager
          closed={showAddManager}
          details={selectedInsurer}
          toggle={() => setshowAddManager(!!0)}
        />
      </Drawer>
    </div>
