...
const Offerbuttons = ({ offer, state, insurer }) => {
  ...
  const [removePayment] = useMutation(REMOVE_PAYMENT, {
    refetchQueries: [
      { query: INSURER, variables: { id: insurer?.insurer?.insurer_id } },
    ],
  });
  ...
  const handleGenerateReceipt = (payment) => {
    window.open(
      `${BASE_URL_LOCAL}/payment_receipt/${btoa(
        JSON.stringify({
  ...
  const handlePaymentSchedule = (payment, id) => {
    ...
    const ids = selectedOFfer.offer_payment
      .slice(id, selectedOFfer.offer_payment.length)
      .map((el) => el.offer_payment_id);
    // alert(JSON.stringify(ids));
    // return

    window.open(
      `${BASE_URL_LOCAL}/payment_schedule/${btoa(
        JSON.stringify({
          offer_id: selectedOFfer?.offer_id,
  ...
  useEffect(() => {
    if (selectedOFfer) {
      ...
      if (selectedOFfer.offer_participant.length > 0) {
        ...
        if (
          selectedOFfer.offer_participant[0].offer_participant_payment.length >
          0
        ) {
          if (
            selectedOFfer.offer_participant[0].offer_participant_payment[
    ...
    if (selectedOFfer) {
      ...
      selectedOFfer.offer_payment.map((payment, key) => {
        ...
        const row = {
          ...
          payment_amount: `${
            obj?.conversion?.currency || selectedOFfer?.offer_detail?.currency
      ...
      >
        ...
        <Modal.Body>
          <div className="row p-2 d-flex justify-content-end">
            {showBtn && selectedOFfer?.payment_status !== "PAID" ? (
              <button
                onClick={() => {
                  setPaymentsModal(!paymentsModal);
