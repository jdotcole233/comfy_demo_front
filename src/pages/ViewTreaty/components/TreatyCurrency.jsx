...
const TreatyCurrency = ({ treaty }) => {
  ...
  const [currencies, setCurrencies] = useState(() => {
    const firstCurrency = JSON.parse(treaty?.treaty_details)?.find(
      (el) => el.keydetail === "currency"
    )?.value;
    if (firstCurrency) {
      return [
        createOption(
          Object.values(list).find((eel) => eel.code === firstCurrency)?.name,
          firstCurrency
        ),
      ];
    }
  ...
  useQuery(TREATY_ACCOUNTS, {
    ...
    onCompleted: (data) => {
      setAlreadyCreatedCurrencies(
        data.fetchTreatyAccounts.map((el) => el.currency)
      );
    },
  });
      ...
      if (willDelete) {
        duplicate({
          ...
          .then(() => {
            ...
            setCurrencies([]);
            swal(
              `Treaty Program has been created for the following currencies: ${currencies
                ?.map((curr) => curr.label)
                .join(", ")}`,
              { icon: "success" }
            );
          ...
          <div className="row">
            <div className="col-md-12">
              <h2>
                ...
                {_.first(currencies)?.value ?? "N/A"}
              </h2>
            </div>
            <div className="col-md-12 ">
              {alreadyCreatedCurrencies.length ? (
                <div className="alert alert-warning">
                  <p>Duplication already made for the following currencies.</p>
                  <ul>
                    {alreadyCreatedCurrencies.map((el) => (
                      <li key={el}>{el}</li>
                    ))}
                  </ul>
                  <p>
                    To enable effective cloning of{" "}
                    {treaty?.treaty_program?.treaty_name} to various selected
                    currencies, Ensure:
                  </p>
                  <p>
                    {" "}
                    1. You have created all re insurers list and allocated their
                    respective quotas.
                  </p>
                  <p>
                    {" "}
                    2. You have created all intermediaries and allocated their
                    respective quotas.
                  </p>
                  <p>
                    This event will automate creation of{" "}
                    {treaty?.treaty_program?.treaty_name} for all currency
                    choices.
                  </p>
                </div>
              ) : null}
